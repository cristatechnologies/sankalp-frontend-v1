...
function CheakoutPage() {
  ...
  const [ccAvenueStatus, setCCAvenue] = useState(null);
   ...
   useEffect(() => {
     if (cart && cart.cartProducts) {
       ...
       console.log("Cart products are not available.");
     }
   }, [cart]);
   // parseInt(item.qty)
   useEffect(() => {
      ...
      if (selectedBilling && selectedShipping) {
        ...
        if (selectPayment) {
          if (selectPayment && selectPayment === "cashOnDelivery") {
           ...
           const url = `${
             process.env.NEXT_PUBLIC_BASE_URL
           }api/user/checkout/order-store-common?token=${
             auth().access_token
           }&shipping_method_id=${parseInt(
             "3"
           )}&shipping_address_id=${selectedShipping}&coupon=${
             couponCode && couponCode.code
           }&billing_address_id=${selectedBilling}&payment_method=Razorpay`;
            ...
            if (response.status == 200) {
              try {
                ...
                console.log("order is ", response.data.order.order_id);
                const inititalResponse = response;
                ...
                const data = await fetch("/api/razorpay", {
                  ...
                  body: JSON.stringify({
                    ...
                    secret_key: response.data.razorpay.secret_key,
                    order_id: response.data.order.order_id,
                    notes: response.data.order,
                    currency: response.data.razorpay.currency_code,
                ...
                console.log(data);
                var options = {
                  key: response.data.razorpay.key, // Enter the Key ID generated from the Dashboard
                  ...
                  handler: async function (response) {
                    // After successful payment, send POST request with payment details to backend
                    const postData = {
                      auth_token: auth_token,
                      key: inititalResponse.data.razorpay.key,
                      secret_key: inititalResponse.data.razorpay.secret_key,
                    ...
                    try {
                      // Send the payment data to the server for verification/processing
                      const result = await fetch("/api/fetchRazorpay", {
                        method: "POST",
                        headers: {
                      ...
                      if (result.ok) {
                        ...
                        // Redirect or show a success message based on result
                        router.push("/profile#order"); // or any success page
                      } else {
                        console.error(
                      ...
                      console.error(
                        "Error sending payment data to server:",
                        error
                      );
                    }
                  },
          ...
          } else if (selectPayment && selectPayment === "phonePe") {
            
          } 
          else if (selectPayment && selectPayment === "ccAvenue") {
            const paymentCCAvenue = () => {
              let paymentData = {
                merchant_id: "36262", // Merchant ID (Required)
                ...
                redirect_url: `${process.env.NEXT_PUBLIC_BASE_URL}/profile#order`, // Success URL (Required)
                cancel_url: `${process.env.NEXT_PUBLIC_BASE_URL}/payment-fail`, // Failed/Cancel Payment URL (Required)
              ...
              let accessCode = "AVQK02BG12CN14KQNC ";
  ...
  const checkDeliveredAddress = async (pincode) => {
    if (pincode) {
      await axios({
        method: "get",
        url: `${process.env.NEXT_PUBLIC_BASE_URL}api/delhivery-pincode?filter_codes=${pincode}`,
      })
        .then((res) => {
                              ...
                              {ccAvenueStatus && (
                                ...
                                >
                                  <div className="w-full flex justify-center">
                                    <span>
                                      <Image
                                        src="/assets/images/PaymentIcons/ccavenue-logo-freelogovectors.net_.png"
                                        width={200}
                                        height={30}
                                      />
                                    </span>
